...
import random
from Locust.files.plate_pic_map import chinese_plate_map, foreign_plate_map


def identify_plate_randomly(client):
    """调用FindCarServer识别库测试接口，图片存放路径为FindCarServer根目录下图片地址"""

    image_path = random.choice(list(foreign_plate_map.keys()))  # 随机选择一张图片
    target_plate = foreign_plate_map[image_path]    # 期望车牌号
    result = None
    flag = None
    result = client.get("/device-access/tool/thirdRecognition",
                        params={'imgPath': '/opt/gx64/ADI_Demo/demo_images/' + image_path})
    ...
    if result.status_code != 200:
        ...
        recognition_color = result.json()['data']['plateColor']     # 车牌识别颜色（暂时没有校验颜色）
        if recognition_plate != '':
            recognition_plate_pure = result.json()['data']['plate']   # 识别出的车牌
            if recognition_plate_pure == target_plate:
                flag = 1    # 1：车牌识别正确
            else:
                flag = 2    # 2：车牌识别错误
        else:
...
if __name__ == '__main__':
    host = '192.168.24.199'
    image_path = random.choice(list(foreign_plate_map.keys()))
