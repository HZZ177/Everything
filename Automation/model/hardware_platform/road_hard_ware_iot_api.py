...
"""
===================================
@FileName: road_hard_ware_iot_api.py
@Description: 请求硬件出入车接口
@Author: huangjunhao
@Software: PyCharm
@Version: 1.0
@Update:
...
def send_road_hard_ware_iot_api(yaml_path, model, header_data, test_data, custom_param_type=0, timeout=10):
    """
    请求硬件云平台接口
    ...
    for query_data in datas:
        if query_data[0]['id'] == 1:
            ...
            if custom_param_type:
                ...
                raise Exception(f"路侧硬件出入车接口测试暂不支持，请求方式为【{method.lower()}】的接口进行测试！")
            return res
