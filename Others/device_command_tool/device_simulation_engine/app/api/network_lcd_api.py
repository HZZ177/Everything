...
from ..services.network_lcd_service import NetworkLcdService
from ..services.device_manager import DeviceManager

# 创建蓝图对象
network_lcd_bp = Blueprint("network_lcd", __name__)
...
# API 路由
@network_lcd_bp.route('/connect', methods=['GET'])
@handle_exceptions
def connect():
    """
    尝试连接设备到服务器
...
@network_lcd_bp.route('/disconnect', methods=['GET'])
@handle_exceptions
def disconnect():
    """
    断开服务器连接
...
@network_lcd_bp.route('/startHeartbeat', methods=['GET'])
@handle_exceptions
def start_heartbeat():
    """
    开启持续心跳
...
@network_lcd_bp.route('/stopHeartbeat', methods=['GET'])
@handle_exceptions
def stop_heartbeat():
    """
    停止心跳
...
@network_lcd_bp.route('/getHistoryCommandMessage', methods=['POST'])
@handle_exceptions
def get_history_command_message():
    ...
    :return:
    """
    logger.info("LCD一体屏getCommandMessage接口被调用")
    # 校验必填参数
    data = request.json
